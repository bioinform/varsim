...
class VarSimVCFComparator(VCFComparator):
    ...
    def run(self):
        ...
        :return:
        '''
        cmd = ['java', JAVA_XMX, '-jar', utils.VARSIMJAR, 'vcfcompare',
...
class RTGVCFComparator(VCFComparator):
    def run(self):
        ...
        cmd = ['java', JAVA_XMX, '-jar', utils.RTGJAR, 'vcfeval',
...
def generate_sdf(reference, log):
    ...
    if os.path.exists(sdf):
        ...
        return sdf
    cmd = ['java', JAVA_XMX, '-jar',utils.RTGJAR,'format',
           '-o', sdf, reference]
    if log:
...
def summarize_results(prefix, tp, fn, fp, var_types = ['SNP', 'Deletion', 'Insertion', 'Complex'], sv_length = 100):
    ...
    :return:
    '''
    cmd = ['java', JAVA_XMX, '-jar', utils.VARSIMJAR, 'vcfcompareresultsparser',
           '-prefix', prefix, '-tp',tp,
           '-fn', fn, '-fp', fp,
